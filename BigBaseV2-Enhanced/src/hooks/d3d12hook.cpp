	...
	long __fastcall hookPresentD3D12(IDXGISwapChain3* pSwapChain, UINT SyncInterval, UINT Flags)
	{
		if (!H::bShuttingDown)
		{
			if (SUCCEEDED(pSwapChain->GetDevice(__uuidof(ID3D12Device), (void**)&d3d12Device))) 
			{
				...
				auto fonts = io.Fonts;
				fonts->AddFontFromFileTTF("c:/windows/fonts/msyh.ttc", 15.0f, NULL, io.Fonts->GetGlyphRangesChineseFull());
				...
				sdesc.OutputWindow = big::g_pointers->m_gta.m_hwnd;
				sdesc.Windowed = ((GetWindowLongPtr(big::g_pointers->m_gta.m_hwnd, GWL_STYLE) & WS_POPUP) != 0) ? false : true;
				...
				ImGui_ImplWin32_Init(big::g_pointers->m_gta.m_hwnd);
